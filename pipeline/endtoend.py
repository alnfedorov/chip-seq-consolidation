...
from .meta import ExperimentMeta, BamMeta
...
async def _process_bam(expfolder: str, experiment: str, meta: BamMeta, maxthreads: int, force: bool):
    ...
    if not meta.simulated:
        ...
        assert os.path.exists(meta.unfiltered)

    # # Mark duplicates, sort and filter.
    # # Not used right now, to save time and memory
    ...
    #     await bam.filter.toduplicates(meta.unfiltered, meta.paired, True, saveto=meta.duplicated, maxthreads=maxthreads)
    #     coro.append(asyncio.create_task(
    ...
    if not os.path.exists(meta.unique) or force:
        await bam.filter.tounique(meta.unfiltered, meta.paired, True, saveto=meta.unique, maxthreads=maxthreads)
        coro.append(asyncio.create_task(
...
async def _process_experiments(expfolder: str, experiments: [ExperimentMeta], maxthreads: int, force: bool):
    ...
    threads = int(round(maxthreads / len(coro)))
